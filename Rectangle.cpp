...
#include "Point2D.h"
		...
		if ( check(vertices) == false){
			throw std::invalid_argument("Los vertices no conforman un rectángulo válido");
		}
		...
		if ( ind < 0 || ind >= N_VERTICES){
			throw std::out_of_range("El vértice no se encuentra el rectángulo");
			}
		return vs[ind];
		}
